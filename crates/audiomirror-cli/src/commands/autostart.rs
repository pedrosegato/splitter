use audiomirror_core::observability::autostart::{write_artifact, AutostartArtifact};
use std::path::Path;
...
fn build_artifact_inner(binary: &Path) -> anyhow::Result<AutostartArtifact> {
    use audiomirror_core::observability::autostart::macos_plist;
...
#[cfg(target_os = "linux")]
fn build_artifact_inner(binary: &Path) -> anyhow::Result<AutostartArtifact> {
    use audiomirror_core::observability::autostart::linux_systemd_unit;
...
fn build_artifact_inner(binary: &Path) -> anyhow::Result<AutostartArtifact> {
    use audiomirror_core::observability::autostart::windows_run_key_entry;
...
fn activate(artifact: &AutostartArtifact) -> anyhow::Result<()> {
    ...
    #[cfg(target_os = "linux")]
    {
        let _ = std::process::Command::new("systemctl")
            ...
            .status();
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "enable", "audiomirror.service"])
            .status();
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "start", "audiomirror.service"])
            .status();
    }
    #[cfg(target_os = "windows")]
    {
...
fn deactivate(_artifact: &AutostartArtifact) -> anyhow::Result<()> {
    #[cfg(target_os = "macos")]
    {
        let uid = nix_uid_or_zero();
        let _ = std::process::Command::new("launchctl")
            .args(["bootout", &format!("gui/{uid}/com.audiomirror.daemon")])
            .status();
    }
    #[cfg(target_os = "linux")]
    {
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "disable", "audiomirror.service"])
            .status();
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "stop", "audiomirror.service"])
            .status();
    }
    #[cfg(target_os = "windows")]
    {
...
mod tests {
    ...
    fn build_artifact_returns_expected_path() {
        ...
        let binary = std::path::PathBuf::from("/usr/local/bin/audiomirror-cli");
