...
impl MetricsRegistry {
    pub fn new() -> Result<Self, NetError> {
        ...
        let packets_sent = IntCounterVec::new(
            Opts::new(
                "audiomirror_stream_packets_sent_total",
        ...
        let packets_received = IntCounterVec::new(
            Opts::new(
                "audiomirror_stream_packets_received_total",
        ...
        let packets_lost = IntCounterVec::new(
            Opts::new(
                "audiomirror_stream_packets_lost_total",
        ...
        let rtt_ms = GaugeVec::new(
            Opts::new("audiomirror_stream_rtt_ms", "RTT in ms per stream"),
            &["stream_id"],
        )
        .map_err(|e| NetError::ConfigIo(format!("prom: {e}")))?;
        let bitrate_kbps = GaugeVec::new(
            Opts::new(
                "audiomirror_stream_bitrate_kbps",
        ...
        .map_err(|e| NetError::ConfigIo(format!("prom: {e}")))?;
        let peers_connected =
            prometheus::Gauge::new("audiomirror_peers_connected", "currently connected peers")
                .map_err(|e| NetError::ConfigIo(format!("prom: {e}")))?;
        let sessions_active =
            prometheus::Gauge::new("audiomirror_sessions_active", "active sessions")
                .map_err(|e| NetError::ConfigIo(format!("prom: {e}")))?;
...
mod tests {
    ...
    fn registry_renders_with_zero_counters() {
        ...
        assert!(raw.contains("audiomirror_peers_connected"));
        assert!(raw.contains("audiomirror_sessions_active"));
    }
    ...
    fn incrementing_counter_appears_in_render() {
        ...
        let raw = m.render().unwrap();
        assert!(raw.contains("audiomirror_stream_packets_sent_total{stream_id=\"3\"} 7"));
    }
}
