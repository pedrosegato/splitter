fn main() {
    #[cfg(target_os = "macos")]
    {
        ...
        println!(
            "cargo:rustc-link-arg-bin=splitter-cli=-Wl,-sectcreate,__TEXT,__info_plist,{plist}"
        );
        add_swift_rpath();
    }
}
...
fn add_swift_rpath() {
    ...
    println!("cargo:rustc-link-arg-bin=splitter-cli=-Wl,-rpath,/usr/lib/swift");
    ...
    if let Ok(output) = Command::new("xcode-select").arg("-p").output() {
        if output.status.success() {
            ...
            for dir in &dirs {
                println!("cargo:rustc-link-arg-bin=splitter-cli=-Wl,-rpath,{dir}");
            }
        }
    }
}
