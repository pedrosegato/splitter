...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.splitter.daemon</string>
...
"#,
    ...
    AutostartArtifact {
        path: home.join("Library/LaunchAgents/com.splitter.daemon.plist"),
        contents: plist,
    }
}
...
"#,
    ...
    AutostartArtifact {
        path: config_home.join("systemd/user/splitter.service"),
        contents: unit,
    }
}
...
pub fn windows_run_key_entry(binary_path: &Path) -> AutostartArtifact {
    AutostartArtifact {
        path: PathBuf::from(r"HKCU\Software\Microsoft\Windows\CurrentVersion\Run\Splitter"),
...
mod tests {
    ...
    fn macos_plist_contains_label_and_binary() {
        let a = macos_plist(
            &PathBuf::from("/usr/local/bin/splitter-cli"),
            &PathBuf::from("/Users/p"),
        );
        assert!(a.contents.contains("com.splitter.daemon"));
        assert!(a.contents.contains("/usr/local/bin/splitter-cli"));
        assert!(a
            .path
            .ends_with("Library/LaunchAgents/com.splitter.daemon.plist"));
    }

    #[test]
    fn linux_unit_contains_execstart() {
        let a = linux_systemd_unit(
            &PathBuf::from("/usr/bin/splitter-cli"),
            &PathBuf::from("/home/p/.config"),
        );
        assert!(a.contents.contains("ExecStart=/usr/bin/splitter-cli"));
        assert!(a.path.ends_with("systemd/user/splitter.service"));
    }

    #[test]
    fn windows_run_key_contains_daemon_arg() {
        let a = windows_run_key_entry(&PathBuf::from(
            r"C:\Program Files\Splitter\splitter-cli.exe",
        ));
