...
PEER_NAME="${1:-alice}"
PORT="${2:-7001}"
IDENTITY_DIR="$HOME/.splitter/$PEER_NAME"
...
./target/release/splitter-cli devices
echo

echo "[3/3] launching daemon..."
echo "press Ctrl+C inside daemon for graceful shutdown."
echo
exec ./target/release/splitter-cli daemon --signaling-port "$PORT" --peer-name "$PEER_NAME" --identity-dir "$IDENTITY_DIR"
